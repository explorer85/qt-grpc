...
def makeNamespaceIdent(packageIdent):
    return '::'.join(packageIdent)

typeIdent = ZeroOrMore(Word(alphas) + Suppress('.'))("namespace").setParseAction(makeNamespaceIdent) + Word(alphas)("typename")
...
methodDefn = (RPC_ - ident("methodName") + 
              LPAR + Optional(STREAM_("paramStreamQualifier")) + Optional(typeIdent("methodParam")) + RPAR +
              RETURNS_ + LPAR + Optional(STREAM_("returnStreamQualifier")) + Optional(typeIdent("methodReturn")) + RPAR + ((LBRACE + RBRACE) | SEMI))
...
def parseProtoFile(path):
    ...
    for smth in package:
        ...
        for r in smth['RPCs']:
            ...
            par = r['methodParam']
            if  par.get('namespace'):
                rpc.paramnamespace = par.get('namespace')
            else:
                rpc.paramnamespace = packageName
            rpc.paramtype = par.get('typename', '')

            returnparam = r['methodReturn']
            if  returnparam.get('namespace'):
                rpc.returnnamespace = returnparam.get('namespace')
            else:
                rpc.returnnamespace = packageName
            rpc.returntype = returnparam.get('typename', '')
...
#pckg, services, messages = parseProtoFile('/home/oius/work/git/core/qt-grpc/examples/protos/binarychat.proto')
#print(pckg)
